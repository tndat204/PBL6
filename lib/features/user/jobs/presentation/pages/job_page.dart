// file: features/user/jobs/presentation/pages/job_page.dart
    ...
    try {
      ...
      if (refresh || _uiCategories.length <= 1) {
        ...
        _uiCategories.insert(
          0,
          CategoryUiModel(
            category: const Category(id: _allCategoriesId, name: 'Tất cả', skills: []),
            isSelected: true,
          ),
        );
        ...
        jobs.map((job) async {
          try {
            ...
            if (company.address != null && company.address!.isNotEmpty) {
                 final parts = company.address!.split(',').map((e) => e.trim()).toList();
                 if (parts.isNotEmpty) {
                    // Try matching the last part with loaded provinces
                    final potentialProvince = parts.last;
                    final provinceExists = _provinces.any((p) => p['name'] == potentialProvince);
                    if (provinceExists) {
                       provinceName = potentialProvince;
                        ...
                        final secondProvinceExists = _provinces.any((p) => p['name'] == secondLast);
                         if (secondProvinceExists) {
                           provinceName = secondLast;
                         }
                    }
                    // If still null, leave it null or assign the full address as fallback?
                    // provinceName ??= company.address; // Optional fallback
                 } else {
                     provinceName = company.address; // If no comma, assume it might be the province
                 }
            }
            ...
            return job.copyWith(
              ...
              // Note: copyWith doesn't modify jobType, expiryDate etc.
              // These should be correctly parsed in Job.fromJson
            );
          } catch (e) {
      ...
      _filteredJobs = _allJobs.where((job) {
        ...
        final matchQuery = (cleanQuery == null || cleanQuery.isEmpty)
            ...
            : (job.title.trim().toLowerCase().contains(cleanQuery) ||
               (job.companyName?.trim().toLowerCase().contains(cleanQuery) ?? false));

        // Salary (Compare int with double?)
        final matchSalary =
            (_filterMinSalary == null || job.salaryMin >= _filterMinSalary!) && // Direct int comparison is fine if filter is double
            (_filterMaxSalary == null || job.salaryMax <= _filterMaxSalary!);

        // ✅ Location Filter (Check if job.location contains the province name)
        final matchLocation = (_filterLocationProvince == null ||
             job.location.toLowerCase().contains(_filterLocationProvince!.toLowerCase()));
             // This is less accurate if job.location is just "Hanoi" but filter is "Thanh pho Ha Noi"
             // A better approach requires structured location data (province name/ID) in the Job object.

        // Job Type (Compare enum.name with String?)
        final matchJobType = (_filterJobType == null || job.jobType.name == _filterJobType);
        ...
        // Combine
        return matchCategory && matchQuery && matchSalary && matchLocation && matchJobType && matchExpiry;
      }).toList();
    });
  }
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const Padding(
                                ...
                                child: Text('Danh mục', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                              ),
                              ...
                              const Padding(
                                padding: EdgeInsets.symmetric(horizontal: 16),
                                child: Text('Công việc phù hợp', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.black87)),
                              ),
                              const SizedBox(height: 16),
