...
const AdminDashboard = () => {
  ...
  // Menu Items dựa trên Use Case Diagram
  const menuItems = [
    { name: 'Tổng quan', icon: <LayoutDashboard size={20} />, active: true },
    { name: 'Quản lý người dùng', icon: <Users size={20} />, active: false },
    { name: 'Quản lý công ty', icon: <Building2 size={20} />, active: false },
    { name: 'Tin tuyển dụng', icon: <Briefcase size={20} />, active: false }, // Use Case: Quản lý toàn bộ tin tuyển dụng
    { name: 'Đánh giá & Review', icon: <Star size={20} />, active: false }, // Use Case: Quản lý toàn bộ đánh giá
    { name: 'Kỹ năng & Danh mục', icon: <Tags size={20} />, active: false }, // Use Case: Quản lý kỹ năng
  ];
    ...
    <div className="flex h-screen bg-gray-100 font-sans">
      ...
      <aside className={`bg-slate-900 text-white transition-all duration-300 ${isSidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        ...
        <nav className="flex-1 py-6">
          <ul className="space-y-2 px-3">
            {menuItems.map((item, index) => (
              <li key={index}>
                <a href="#" className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-colors ${item.active ? 'bg-blue-600 text-white' : 'text-slate-400 hover:bg-slate-800 hover:text-white'}`}>
                  {item.icon}
        ...
        <div className="p-4 border-t border-slate-700">
          <button className="flex items-center gap-4 px-4 py-2 text-red-400 hover:text-red-300 transition-colors w-full">
            <LogOut size={20} />
        ...
        <header className="h-16 bg-white shadow-sm flex items-center justify-between px-6">
          ...
          <div className="flex items-center gap-6">
            <div className="relative hidden md:block">
                ...
                <input type="text" placeholder="Tìm kiếm..." className="pl-10 pr-4 py-2 rounded-full bg-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 w-64" />
            </div>
